...
class UserManger(BaseUserManager):
    def create_user(self, first_name, last_name, username, email, password=None):
        ...
        # where to save users in db using= self._db
...
class UserProfile(models.Model):
    ...
    date_joined = models.DateTimeField(auto_now_add=True)
    latitude = models.DecimalField(max_digits=30, decimal_places=8, blank=True, null=True)
    longitude = models.DecimalField(max_digits=30, decimal_places=8, blank=True, null=True)
    modified_date = models.DateTimeField(auto_now=True)

    def __str__(self):
        return self.user.email

# use django signal to create user profile
